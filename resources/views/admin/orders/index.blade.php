...
<div class="search">
    ...
    <input placeholder="Поиск..." data-href="orders" type="search" data-token="{{ csrf_token() }}" id="search">
</div>
<br>
...
<ul class="paginate" id="paginate">
    @if($orders->lastPage() > 1)
        @for($i = 1; $i <= $orders->lastPage() && $i <=20;$i++)
            <div data-page="{{$i}}" class="paginate-item @if ($orders->currentPage() == $i)  {{"active"}}  @endif">{{$i}}</div>
        @endfor
    @endif
</ul>
...
<table id="list-component" class="table-elements" data-list="brands">
    ...
    @forelse ($orders as $item)
        <tr>
            <td>Заказ №{{ $item->order_id }}</td>
            <td>{{$item->status['active']['name']}}</td>
            <td>{{date('d-m-Y', strtotime($item->created_at))}}</td>

            <td style="width: 15px; text-align: center;">
                <a class="edit-link" href="{{route('orders.edit',[$item->order_id])}}">
            ...
            <td style="width: 15px; text-align: center;">
                <a class="delete-element delete-link" href="{{route('orders.destroy',[$item->order_id])}}">
    ...
    @empty
        <tr>
            <td colspan="5">Заказов нет нет</td>
        </tr>
    @endforelse
    </tbody>
</table>
