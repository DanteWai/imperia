...
@forelse ($brands as $item)
    <li class="group-elements-item">
        @if(isset($item->brand_logo['min']))
            <img src="{{'/public/images/brands/'.$item->brand_logo['min']}}" alt="">
        @else
            <img src="/public/images/admin/noimg.png" alt="">
        @endif
        <h3 class="group-elements-item_title">{{ $item->brand_name }}</h3>

        <div class="group-elements-item_footer">
            <span class="group-elements-item_date">{{date('d-m-Y', strtotime($item->created_at))}}</span>
...
@empty
    <p>Производителей нет</p>
@endforelse
</ul>
-->

<table id="page-list3" class="table-elements" data-list="brands">
    <thead>
        <tr>
            <td>Название</td>
            <td>Изображение</td>
